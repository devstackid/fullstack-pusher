    ...
    return (
        ...
        >
            ...
            <div className="mx-auto flex flex-col gap-3 px-2 py-4 text-center">
                <img src={user.avatar} alt={user.name} className="mx-auto h-20 w-20 rounded-full border border-secondary" />
                <h5 className="font-medium">{user.name}</h5>
            </div>

            <div className="px-2">
                <button className="flex w-full items-center justify-between rounded-lg px-3 py-2 hover:bg-secondary focus:bg-secondary" onClick={() => setToggleCustomizeChat(!toggleCustomizeChat)}>
                    <span>Customize Chat</span>
                    {toggleCustomizeChat ? <BsChevronDown /> : <BsChevronRight />}
                </button>
                {toggleCustomizeChat && <button className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-secondary focus:bg-secondary" onClick={customizeChat}>
                        <BsRecordCircle className={clsx(!user.message_color && "!text-primary")} style={{ color: user.message_color! }} />
                        Change Theme
                    </button>}

                    <button className="flex w-full items-center justify-between rounded-lg px-3 py-2 hover:bg-secondary focus:bg-secondary" onClick={() => setToggleShowMedia(!toggleShowMedia)}>
