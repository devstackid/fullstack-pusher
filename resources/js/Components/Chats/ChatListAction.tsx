...
import { BsArchive, BsCheck, BsCheck2, BsThreeDots, BsXLg } from "react-icons/bs"
...
const Action = ({chat}: ActionProps) => {
    ...
    return <div ref={dropdownRef}>
        <Dropdown.Trigger>
            <button type="button" className={clsx('rounded-full border border-secondary bg-background p-1.5 shadow-sm group-hover:visible group-hover:flex', open ? 'visible' : 'invisible')}>
                <BsThreeDots className="text-secondary-foreground" />
            </button>
        </Dropdown.Trigger>
        <Dropdown.Content align={dropdownPosition ? 'right' : 'top-right'} contentClasses={dropdownPosition ? "" : "mb-7"}>
            ...
            {auth.id !== chat.id && auth.id !== chat.from_id && (
                ...
                <div className="flex items-center gap-2 text-xs">
                    ...
                    Tandai {chat.is_read ? 'belum dibaca' : 'telah dibaca'}
                </div>
            </Dropdown.Button>
            )}
