...
export default function ChatMessageAttachment({message, messageWithImages, messageWithFiles, dir = "ltr", className, gridClassName, deleteMessageClassName}: ChatMessageAttachmentProps) {
  ...
  return message.attachments && message.attachments.length > 0 && (
    <div className="group relative flex gap-1">
        <div className={clsx("flex max-w-xs flex-col", className)}>
            {messageWithImages.length > 0 && <div
                className={clsx(
                    "grid",
                    gridClassName,
                    messageWithImages.length >=
                        3
                        ? "w-[300px] grid-cols-3"
                ...
                {messageWithImages.map(
                    (attachment) => (
                        ...
                        >
                            ...
                            {message.attachments.length > 1 && <DeleteSelectedFileInChat message={message} attachment={attachment} /> }
            ...
            {messageWithFiles.length > 0 && <div className="ml-auto grid max-w-xs grid-cols-1 gap-1">
                {messageWithFiles.map(
                    (attachment) => (
                        <div
                            key={
                                attachment.file_name
        ...
        {!message.body && <DeleteMessage message={message} className={clsx('my-auto ml-auto mr-2', deleteMessageClassName)} />}
    </div>
)
}
