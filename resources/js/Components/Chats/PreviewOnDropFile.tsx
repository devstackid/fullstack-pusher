...
export default function PreviewOnDropFile({
    ...
    const removeAttachment = (file: Preview) => {
        ...
        const removedIndex = attachments.findIndex((f) => f.preview === file.preview)
                ...
                <div className="mt-auto flex gap-1 overflow-auto">
                    {attachments.map((file, index) => (
                        <div key={`${file.name}-${index}`} className="group relative">
                            <button className={clsx('flex h-16 w-16 shrink-0 overflow-hidden rounded-lg border-2 border-secondary bg-secondary transition-all hover:border-primary focus:border-primary', selectedPreview.preview === file.preview ? 'border-primary' : 'border-transparent')} onClick={() => changeSelectedImage(file)}>
                            {isImageLinkValid(file.name) ? <img
                                    src={file.preview}
                                    alt=""
                                    className="h-full w-full object-cover"
                                /> : <span className="flex h-[56px] w-[56px] shrink-0 items-center justify-center"><BsFileEarmarkText className="text-3xl" /></span>}
                                
                            </button>

                            <button onClick={() => removeAttachment(file)} className="absolute right-1 top-1 z-10 hidden h-4 w-4 items-center justify-center rounded-full bg-danger text-white group-hover:flex">
                                <BsX />
                            </button>
                        </div>
                    ))}
